...
int main(int argc, char *argv[]){
	...
	if (ipcMethod == "f") {
		RequestChannel* control_channel = new FIFORequestChannel ("control", RequestChannel::SERVER_SIDE);
		handle_process_loop (control_channel);
	} else if (ipcMethod == "q") {
		RequestChannel* control_channel = new MQRequestChannel ("control", RequestChannel::SERVER_SIDE, buffercapacity);
		handle_process_loop (control_channel);
	} else if (ipcMethod == "s") {
		RequestChannel* control_channel = new SHMRequestChannel ("control", RequestChannel::SERVER_SIDE, buffercapacity);
		handle_process_loop (control_channel);
	} else {
		cout << "This IPC method does not exist (server)." << endl;
	}
}
